...
backgroundImage.x  = 0;
backgroundImage.y = 0;  
staticLevelContainer.height = 3000;
staticLevelContainer.width = 3000;
...
function loadLevel(filename:string) {
    ...
    backgroundImage.height = 3000
    backgroundImage.width = 3000

    let scaleX = 3000/window.innerWidth;
    let scaleY = 3000/window.innerHeight;
    console.log(filename);

    engine.stage.scale = new PIXI.Point(scaleX*1.5, scaleY*1.5);

    if (playerInfo.displayType == "hitbox") {
...
function create() {
    ...
    window.addEventListener("keydown", async function (event) {
        ...
        engine.stage.scale = new PIXI.Point((3000/window.innerWidth)*1.5, (3000/window.innerHeight)*1.5);

        }

        if (event.key === "f") {
            playerInfo.fly = !playerInfo.fly;
            saveCookie();
        }
...
let camHeight:number = (3000/window.innerHeight)*-1.5; // 
let camWidth:number = (3000/window.innerWidth)*-1.5;
function update() {
    fpsMeter.updateTime();
    
    if (playerInfo.dev && playerInfo.fly) {
        console.log("devmode");
...
if (playerInfo.leftWall) {
    ...
    for (let i = 0; i < levelData.respawnPoint.length; i++) {
        if (player.x + player.width > levelData.respawnPoint[i][0]
            ...
            {
                console.log("respawn point set", [levelData.respawnPoint[i][0], levelData.respawnPoint[i][1]]);
    ...
    let playeryplayerheightplayerInfovyflip1 = player.y+player.height+(playerInfo.vy*flip)-1
    ...
    for (var i = 0; i < levelData.phyBox.length; i++) {
        if (!leftwallfound && player.y >= levelData.phyBox[i][0][1] 
            && playeryplayerheightplayerInfovyflip1 <= levelData.phyBox[i][1][1] 
            ) { 
            if (player.x - 6 < levelData.phyBox[i][1][0] && player.x - 6 > levelData.phyBox[i][0][0]) {
                playerInfo.leftWall = true;

                player.x = levelData.phyBox[i][1][0]+3;
                if (playerInfo.vx > 0) {playerInfo.vx = 0;}
                leftwallfound = true;
            }else{playerInfo.leftWall = false;}}
            // |---------------------- cheak right wall --------------------|
        if (!rightwallfound && player.y - 10 >= levelData.phyBox[i][0][1] 
            && player.y +10 <= levelData.phyBox[i][1][1]) { 
            if (player.x + player.width + 6 < levelData.phyBox[i][1][0] 
                && player.x +  player.width + 6 > levelData.phyBox[i][0][0]
